...
If this specification leaves an implementation detail unspecified and the choice cannot materially alter required behavior or conformance, the implementer MAY choose an appropriate solution.

If an omission or ambiguity could materially affect externally observable behavior, data semantics, security, privacy, accessibility, interoperability, safety, or another normative property, the implementer MUST surface the ambiguity rather than silently inventing a product or design rule.
...
This specification is version **0.1.2** and is currently a **draft**.

It conforms to **SPEC.md Standard 0.3.0**. The SPEC.md standard version and this Judo Club specification version are intentionally independent.
...
The root file therefore owns the complete `spec_version` directly.

There are no normative specification modules or normative companion documents required for conformance.

A larger design could decompose its specification into directly referenced modules. Under SPEC.md 0.3.0, the root `SPEC.md` would own the version of that complete Specification Set.
...
## 8.2 Version 0.1.2 Clarification

Version 0.1.2 does not change the required behavior of the Judo Club Website.

It updates the document to SPEC.md Standard 0.3.0 and clarifies the distinction between implementation freedom and material ambiguity. This is therefore a PATCH-level change to this example specification.
