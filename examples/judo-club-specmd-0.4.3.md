...
This specification is version **0.1.5** and is currently a **draft**.

It conforms to **SPEC.md Core 0.4.3**. The Core version and this Judo Club specification version are intentionally independent.
...
This revision aligns the example with SPEC.md Core 0.4.3. Required website behavior is unchanged.

Core 0.4.3 clarifies that stable IDs may identify non-requirement elements and that an ID alone does not make an element normative. This example contains no separate traceability requirements, so no behavioral changes are needed.
...
## 8.2 Version 0.1.5 Clarification

Version 0.1.5 does not change the required behavior of the Judo Club Website.

It updates the document to SPEC.md Core 0.4.3. Required website behavior is unchanged, so this is a PATCH-level change.
