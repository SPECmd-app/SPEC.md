...
companion_to: "SPEC.md Core 0.4.3"
...
**Status:** Draft  
**Companion to:** SPEC.md Core 0.4.3  
**Tagline:** **Portable Ideas & Designs.**
...
A flow identifier identifies a behavioral flow. It does not by itself make the flow a normative requirement.
...
A semantic ID does not by itself make an element a normative requirement.

Behavioral flows and other identified specification elements MAY be traced when useful, but their IDs do not automatically require TRACE coverage.

> **Note:** We are not saying flows are unimportant or cannot be traced. We are saying **TRACE coverage follows normative obligations, not every label in the document**.
...
## 46.1 Alignment with Core 0.4.3

This release aligns Optional with SPEC.md Core 0.4.3.

Core 0.4.3 retains the 0.4.2 design boundary and clarifies that stable IDs may identify non-requirement elements. Identifier syntax alone does not determine normative status or trace coverage.

This Optional release also clarifies the relationship between behavioral-flow IDs and TRACE.md: flows may be traced when useful, but trace coverage follows normative obligations rather than every identified element.
...
The flow appears in this trace chain because it helps explain how the requirement is realized. Its presence does not mean that every identified flow requires a TRACE.md entry.
