...
### Specification Set version ownership

By default, the root `SPEC.md` owns the `spec_version` of the complete Specification Set.

Normative modules are parts of that versioned set and SHOULD NOT carry independent `spec_version` values.

A module MAY identify the set it belongs to:
...
Any normative change in any module is a change to the Specification Set and therefore participates in determining the next root `spec_version` bump.

For example:

- an editorial clarification in one module may require a PATCH bump;
- an additive optional capability in one module may require a MINOR bump;
- a module change that can make a previously conforming implementation non-conforming requires a MAJOR bump.

An independently published and reusable module MAY have its own module version, but that is an advanced use case and is not required by Core.

The purpose of modular composition is to keep a design understandable and maintainable without losing a single authoritative entry point or version authority.
...
If this specification leaves an implementation detail unspecified
and the choice cannot materially alter required behavior or
conformance, the implementer MAY choose an appropriate solution.

If an omission or ambiguity could materially affect externally
observable behavior, data semantics, security, privacy,
accessibility, interoperability, safety, or another normative
property, the implementer MUST surface the ambiguity rather than
silently invent a product or design rule.
```
...
## 3.3 Conflicts, Omissions, and Ambiguity

A specification must distinguish between **implementation freedom** and **material ambiguity**.

### Implementation freedom

If a detail is unspecified and choosing among reasonable implementations cannot materially alter required behavior or conformance, the implementer MAY choose an appropriate solution.

Examples may include:

- internal source-code organization;
- framework choice;
- non-contractual visual styling;
- private helper functions;
- storage mechanics that preserve the specified semantics.

### Material ambiguity

If an omission or multiple plausible interpretations could materially change:

- externally observable behavior;
- domain or data semantics;
- security;
- privacy;
- accessibility;
- interoperability;
- safety;
- lifecycle behavior;
- another normative property;

the implementer MUST surface the ambiguity rather than silently inventing a product or design rule.

### Normative conflict

If two normative statements cannot both be satisfied, the conflict MUST be surfaced rather than silently resolved.
...
5. A more specific module may refine a general rule only within its declared scope.
6. An unresolved normative conflict or material ambiguity MUST be surfaced.

This section defines when an implementer is permitted to decide and when the specification author must clarify.
...
Product/design specification version
Implementation version
```

An implementation release does not require a specification version change unless the specified design or behavior changes.

## 8.5 Specification Set Versioning

When a design uses a modular Specification Set, the root `SPEC.md` owns the version of the complete normative set unless the root explicitly defines another model.

Example:

```text
SPEC.md                    ← spec_version 2.3.0
spec/authentication.md     ← part of 2.3.0
spec/campaigns.md          ← part of 2.3.0
spec/integrations/acme.md  ← part of 2.3.0
```

A change to any normative module MUST be evaluated using the same MAJOR/MINOR/PATCH rules as a change made directly in the root file.

Module files SHOULD NOT independently increment `spec_version` because doing so can create ambiguous conformance targets.

A reusable module maintained as an independently versioned specification MAY declare a separate `module_version`, but the root SPEC.md MUST identify the exact compatible module version or version range required by the Specification Set.
...
An implementer should be free to choose unspecified internal mechanisms when those choices cannot materially change conformance.

The specification should not force the implementer to guess where an unspecified choice could materially affect a normative property.
...
If an unspecified implementation detail cannot materially alter
conformance, the implementer MAY choose an appropriate solution.

If an omission could materially affect behavior, data semantics,
security, privacy, accessibility, interoperability, or another
normative property, the ambiguity MUST be surfaced rather than
silently resolved.
...
Tooling is optional in version 0.3.0.
...
# 14. Draft Status of 0.3.0

Version 0.3.0 is the third draft of the SPEC.md standard. It promotes the material-ambiguity test into Core and defines root-owned versioning for modular Specification Sets.
...
- reusable independently versioned module semantics;
- tooling schemas;
- formal validation rules;
- practical stress-testing of Specification Sets and `TASKS.md` in a larger worked example.
...
<self-describing interpretation rules, including normative language,
implementation freedom, conflict handling, and material ambiguity>
